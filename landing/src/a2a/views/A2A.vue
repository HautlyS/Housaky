    ...
    <div class="card mb-4">
      ...
      <div class="card-body">
        <div class="learning-list">
          <div v-for="learning in displayLearnings" :key="learning.id" class="learning-item">
...
const displayLearnings = computed(() => 
  learnings.value.length > 0 ? learnings.value : defaultLearnings
)
...
async function connectWebSocket() {
  try {
    await store.connectWebSocket((data) => {
      handleIncomingMessage(data)
    })
    wsConnected.value = store.wsConnected
  } catch (e) {
    ...
    store.addMessage({
      ...
      to: 'self',
      type: 'error',
      content: 'A2A Hub unavailable - running in demo mode'
    })
  }
}
