...
async fn test_key_manager_basic_functionality() -> Result<(), anyhow::Error> {
    let config = housaky::config::schema::ReliabilityConfig {
        providers: vec![housaky::config::schema::ProviderConfig {
            name: "test_provider".to_string(),
            models: vec![housaky::config::schema::ModelConfig {
                name: "test_model".to_string(),
                api_keys: vec![housaky::config::schema::ApiKeyConfig {
    ...
    let best_key = key_manager
        .get_best_key("test_provider", "test_model")
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        housaky::config::schema::HealthStatus::Unknown
    );
