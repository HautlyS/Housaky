#!/bin/bash
# Housaky Telegram Integration Test Suite
...
if [ -f "target/release/housaky" ]; then
    BINARY_SIZE=$(ls -lh target/release/housaky | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/housaky 2>/dev/null || stat -c%s target/release/housaky)
...
CONFIG_PATH="$HOME/.housaky/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'housaky onboard' first"
    fi
else
    warn "No config file found - run 'housaky onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/housaky channel doctor 2>&1 || true)
END_TIME=$(date +%s)
