...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/theonlyhennygod/housaky/security/advisories/new)
...
Housaky implements defense-in-depth security:
...
Housaky Docker images follow CIS Docker Benchmark best practices:
...
docker build -t housaky .
docker inspect --format='{{.Config.User}}' housaky
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace housaky gateway
```
