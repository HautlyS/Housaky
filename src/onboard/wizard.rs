...
pub fn run_wizard() -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to Housaky — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 9, "Channels (How You Talk to Housaky)");
    ...
    let config = Config {
        ...
        tls: crate::config::schema::TlsConfig::default(),
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("HOUSAKY_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to Housaky)");
...
/// Use `housaky onboard` or `housaky onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
/// Use `housaky onboard --interactive` for the full wizard.
...
pub fn run_quick_setup(
    ...
    let workspace_dir = housaky_dir.join("workspace");
    ...
    if api_key.is_none() {
        ...
        println!("    2. Or edit:           ~/.housaky/config.toml");
        println!("    3. Chat:              housaky agent -m \"Hello!\"");
        println!("    4. Gateway:           housaky gateway");
    } else {
        println!("    1. Chat:     housaky agent -m \"Hello!\"");
        println!("    2. Gateway:  housaky gateway");
        println!("    3. Status:   housaky status");
    }
    println!();
...
pub fn run_models_refresh(
    ...
    if !force {
        if let Some(cached) = load_cached_models_for_provider(
            ...
            println!();
            println!(
                "Tip: run `housaky models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    let home = directories::UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    let default_dir = home.join(".housaky");
    ...
    let housaky_dir = if use_default {
        default_dir
    } else {
...
fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("Housaky works with ANY API that speaks the OpenAI chat completions format.");
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("Housaky will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how Housaky connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("Housaky uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("Housaky can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("Housaky can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should Housaky interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how Housaky stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to Housaky from anywhere.");
        ...
        match choice {
            0 => {
                ...
                println!(
                    ...
                    style("— talk to Housaky from Telegram").dim()
                );
            ...
            1 => {
                ...
                println!(
                    ...
                    style("— talk to Housaky from Discord").dim()
                );
            ...
            2 => {
                ...
                println!(
                    ...
                    style("— talk to Housaky from Slack").dim()
                );
            ...
            3 => {
                ...
                print_bullet("Housaky reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            5 => {
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("housaky-whatsapp-verify".into())
                    .interact_text()?;
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("Housaky is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if has_channels {
        ...
        println!("       {}", style("housaky channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("housaky agent -m \"Hello, Housaky!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("housaky agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("housaky status").yellow());
...
mod tests {
    ...
    fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = fs::read_to_string(tmp.path().join("IDENTITY.md")).unwrap();
        assert!(
            identity.contains("**Name:** Housaky"),
            "should default agent name to Housaky"
        );
    ...
    fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = fs::read_to_string(tmp.path().join("SOUL.md")).unwrap();
        assert!(soul.contains("Housaky-v2"));
    }
