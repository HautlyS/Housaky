...
impl SkillsPanel {
    ...
    fn inject_demo_skills(&mut self) {
        let demos: &[(&str, &str, bool, SkillSource, &str)] = &[
            (
                "get-shit-done",
                "Productivity & execution: task decomposition, planning, tracking.",
                true,
                SkillSource::Local,
                "1.0.0",
            ),
            (
                "ui-ux-pro-max",
                "Advanced UI/UX design, a11y, design systems, Figma patterns.",
                false,
                SkillSource::Local,
                "1.0.0",
            ),
            (
                "code-review",
                "Deep code review: security, perf, maintainability.",
                false,
            ...
            (
                "data-analysis",
                "Stats, visualization, CSV/JSON data processing.",
                false,
                SkillSource::OpenClaw,
                "1.2.1",
            ),
            (
                "computer-use",
                "Claude official: desktop automation.",
                false,
                SkillSource::ClaudeOfficial,
                "official",
            ),
            (
                "web-search",
            ...
            (
                "memory-graph",
                "Long-term knowledge graph management.",
                true,
                SkillSource::Local,
                "0.4.2",
            ),
            (
                "rust-expert",
                "Advanced Rust: unsafe, async, macro development.",
                false,
                SkillSource::OpenClaw,
                "1.0.0",
            ),
    ...
    pub fn toggle_selected(&mut self) -> Option<bool> {
        ...
        if skill.enabled {
            skill.enabled = false;
            let msg = format!("Disabled: {}", skill.name);
        ...
        skill.enabled = true;
        let msg = format!("Enabled: {}", skill.name);
        self.status = msg.clone();
        self.notification = Some((msg, std::time::Instant::now()));
        Some(true)
    }

    pub fn get_selected_skill_needs_install(&self) -> Option<(String, SkillSource)> {
        let idx = self.selected_real_idx()?;
        let skill = &self.skills[idx];
        if !skill.enabled && !skill.installed {
            Some((skill.name.clone(), skill.source.clone()))
        } else {
            None
        }
    }
    ...
    pub fn mark_selected_installed(&mut self) {
        if let Some(idx) = self.selected_real_idx() {
            self.skills[idx].installed = true;
            self.skills[idx].enabled = true;
            let msg = format!("Installed & enabled: {}", self.skills[idx].name);
            self.status = msg.clone();
            self.notification = Some((msg, std::time::Instant::now()));
        }
    }

    pub fn add_marketplace_skill(
        &mut self,
        name: String,
        description: String,
        source: SkillSource,
    ...
    pub fn load_marketplace_skills(
        &mut self,
        workspace_dir: &std::path::Path,
        config: &crate::config::Config,
    ) {
        if let Ok(claude_skills) =
            crate::skills::marketplace::list_claude_official_plugins(workspace_dir, config)
        {
            for m in claude_skills {
                let enabled = config.skills.enabled.get(&m.name).copied().unwrap_or(false);
                let installed = workspace_dir
                    .join("skills")
                    .join(&m.name)
                    .join("SKILL.toml")
                    .exists();
                if !self.skills.iter().any(|s| s.name == m.name) {
                    self.skills.push(Skill {
                        name: m.name,
                        ...
                        version: m.enabled.to_string(),
                        tags: vec!["claude".to_string(), "marketplace".to_string()],
                        path: None,
                        readme: None,
                    });
                }
        ...
        if let Ok(openclaw_skills) =
            crate::skills::marketplace::list_openclaw_vendored_skills(workspace_dir, config)
        {
            for m in openclaw_skills {
                let enabled = config.skills.enabled.get(&m.name).copied().unwrap_or(false);
                let installed = workspace_dir
                    .join("skills")
                    .join(&m.name)
                ...
                if !self.skills.iter().any(|s| s.name == m.name) {
                    self.skills.push(Skill {
                        ...
                        version: m.enabled.to_string(),
                        tags: vec!["openclaw".to_string(), "marketplace".to_string()],
                        path: None,
                        readme: None,
                    });
                }
    ...
    fn draw_tab_bar(&self, f: &mut Frame, area: Rect) {
        let titles: Vec<Span> = SkillTab::ALL
            .iter()
            .map(|&t| {
                let count = match t {
            ...
            .map(|(di, &ri)| {
                ...
                ListItem::new(vec![Line::from(vec![
                    ...
                    Span::styled(
                        ...
                        ratatui::style::Style::default().fg(s.source.color()).bg(bg),
                    ),
                ])])
            })
            .collect();
    ...
    fn draw_detail(&self, f: &mut Frame, area: Rect) {
        ...
        let info = vec![
            ...
            Line::from(vec![
                ...
                Span::styled(
                    skill.source.label(),
                    ratatui::style::Style::default().fg(skill.source.color()),
                ),
            ]),
            Line::from(vec![
                Span::styled(
        ...
        let gauge = Gauge::default()
            .block(
                Block::default()
                    ...
                    .title(Span::styled(" Marketplace Activation ", style_muted())),
            )
            .gauge_style(
                ratatui::style::Style::default()
                    .fg(Palette::CYAN)
    ...
    fn draw_footer(&self, f: &mut Frame, area: Rect) {
        let hint = if self.filter_active {
            ...
            " ↑↓/jk=navigate  Space/Enter=install  ↓/j=subtab  Tab=main tab  /=filter  r=refresh  PgUp/Dn=scroll "
        };
