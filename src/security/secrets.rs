...
// in `~/.housaky/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.housaky/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(housaky_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: housaky_dir.join(".secret_key"),
            enabled,
        }
    }
