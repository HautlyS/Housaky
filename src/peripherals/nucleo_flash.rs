//! Flash Housaky Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash Housaky Nucleo firmware. Builds from firmware/housaky-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (housaky's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("housaky-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from housaky repo root.",
            firmware_dir.display()
        );
    }

    println!("Building Housaky Nucleo firmware...");
    ...
    println!("Housaky Nucleo firmware flashed successfully.");
