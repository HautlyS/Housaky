//! Flash Housaky Arduino firmware via arduino-cli.
//!
...
/// Housaky Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/housaky-arduino/housaky-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "housaky-arduino";
...
/// Flash Housaky firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("housaky_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling Housaky Arduino firmware...");
    ...
    println!("Housaky firmware flashed successfully.");
