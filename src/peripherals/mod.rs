...
/// Handle `housaky peripheral` subcommands.
#[allow(clippy::module_name_repetitions)]
pub fn handle_command(cmd: crate::commands::PeripheralCommands, config: &Config) -> Result<()> {
    match cmd {
        crate::commands::PeripheralCommands::List => {
            let boards = list_configured_boards(&config.peripherals);
            if boards.is_empty() {
                println!("No peripherals configured.");
                println!();
                println!("Add one with: housaky peripheral add <board> <path>");
                println!("  Example: housaky peripheral add nucleo-f401re /dev/ttyACM0");
                println!();
        ...
        crate::commands::PeripheralCommands::Add { board, path } => {
        ...
        #[cfg(feature = "hardware")]
        crate::commands::PeripheralCommands::Flash { port } => {
