...
pub async fn run(config: Config, host: String, port: u16) -> Result<()> {
    ...
    {
        let housaky_cfg = config.clone();
        handles.push(spawn_component_supervisor(
            "housaky",
            initial_backoff,
            max_backoff,
            move || {
                let cfg = housaky_cfg.clone();
                async move { run_housaky_heartbeat(cfg).await }
            },
        ));
    }
    ...
    // SIGTERM is the standard signal used by systemd/launchd to stop services.
    #[cfg(unix)]
    {
    ...
    tracing::info!("Stopping all daemon components...");

    for handle in &handles {
        handle.abort();
    }
    // Wait for all tasks to acknowledge the abort with a short timeout
    let abort_timeout = Duration::from_secs(5);
    for handle in handles {
        let _ = tokio::time::timeout(abort_timeout, handle).await;
    }

    tracing::info!("Housaky daemon stopped cleanly");
    Ok(())
}
...
async fn run_housaky_heartbeat(config: Config) -> Result<()> {
    let mut agent = crate::Agent::new(&config)?;
    agent.initialize().await?;

    let arc_agent = std::sync::Arc::new(agent);

    let heartbeat = crate::heartbeat::HousakyHeartbeat::new(arc_agent);
