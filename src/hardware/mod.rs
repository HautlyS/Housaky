...
/// Handle `housaky hardware` subcommands.
#[allow(clippy::module_name_repetitions)]
pub fn handle_command(cmd: crate::commands::HardwareCommands, _config: &Config) -> Result<()> {
    ...
    #[cfg(feature = "hardware")]
    match cmd {
        crate::commands::HardwareCommands::Discover => run_discover(),
        crate::commands::HardwareCommands::Introspect { path } => run_introspect(&path),
        crate::commands::HardwareCommands::Info { chip } => run_info(&chip),
    }
}
...
fn run_info(chip: &str) -> Result<()> {
    ...
    #[cfg(not(feature = "probe"))]
    {
        ...
        println!();
        println!("Then run: housaky hardware info --chip {}", chip);
        println!();
