...
pub fn trim_history_with_limit(history: &mut Vec<ChatMessage>, max_messages: usize) {
...
fn trim_history(history: &mut Vec<ChatMessage>) {
    trim_history_with_limit(history, 50);
}

fn build_compaction_transcript(messages: &[ChatMessage], max_chars: usize) -> String {
...
async fn auto_compact_history(
    ...
    let start = if has_system { 1 } else { 0 };
    let keep_count = keep_recent.min(non_system_count);
    let compact_count = non_system_count.saturating_sub(keep_count);
    if compact_count == 0 {
        return Ok(false);
    }
    ...
    let transcript = build_compaction_transcript(&to_compact, max_source_chars);
    ...
    let summary = truncate_with_ellipsis(&summary_raw, max_summary_chars);
...
pub fn parse_tool_calls(response: &str) -> (String, Vec<ParsedToolCall>) {
    ...
    // Fall back to XML-style <invoke> tag parsing (Housaky's original format)
...
pub async fn run(
    ...
    if config.peripherals.enabled && !config.peripherals.boards.is_empty() {
        ...
        tool_descs.push((
            "arduino_upload",
            "Upload agent-generated Arduino sketch. Use when: user asks for 'make a heart', 'blink pattern', or custom LED behavior on Arduino. You write the full .ino code; Housaky compiles and uploads it. Pin 13 = built-in LED on Uno.",
        ));
        tool_descs.push((
            "hardware_memory_map",
    ...
    if let Some(msg) = message {
        ...
        println!("🦀 Housaky Interactive Mode");
        ...
        while let Some(msg) = rx.recv().await {
            ...
            if let Ok(compacted) = auto_compact_history(
                ...
                config.agent.compaction_keep_recent_messages,
                config.agent.compaction_max_source_chars,
                config.agent.compaction_max_summary_chars,
            )
            .await
            {
                if compacted {
            ...
            trim_history_with_limit(&mut history, config.agent.max_history_messages);
...
pub async fn process_message(config: Config, message: &str) -> Result<String> {
    ...
    if config.peripherals.enabled && !config.peripherals.boards.is_empty() {
        ...
        tool_descs.push((
            "arduino_upload",
            "Upload Arduino sketch. Use for 'make a heart', custom patterns. You write full .ino code; Housaky uploads it.",
        ));
        tool_descs.push((
            "hardware_memory_map",
...
{"name": "file_write", "arguments": {"path": "hello.py", "content": "print('hello')"}}"#;
    ...
    fn build_compaction_transcript_formats_roles() {
        ...
        let transcript = build_compaction_transcript(&messages, 12_000);
