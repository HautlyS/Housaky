...
impl Provider for OpenRouterProvider {
    ...
    async fn chat_with_system(
        ...
        let api_key = self.api_key.as_ref()
            .ok_or_else(|| anyhow::anyhow!("OpenRouter API key not set. Run `housaky onboard` or set OPENROUTER_API_KEY env var."))?;
        ...
        let response = self
            ...
            .header("Authorization", format!("Bearer {api_key}"))
            .header("HTTP-Referer", "https://github.com/theonlyhennygod/housaky")
            .header("X-Title", "Housaky")
            .json(&request)
            .send()
            .await?;
    ...
    async fn chat(
        ...
        let api_key = self.api_key.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
            "OpenRouter API key not set. Run `housaky onboard` or set OPENROUTER_API_KEY env var."
        )
        })?;
