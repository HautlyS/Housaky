...
/// Initial backoff on poll/network error (seconds)
const POLL_INITIAL_BACKOFF_SECS: u64 = 2;
/// Maximum backoff on repeated errors (seconds)
const POLL_MAX_BACKOFF_SECS: u64 = 60;
/// Long-poll timeout (seconds) — Telegram allows up to 60s
const POLL_TIMEOUT_SECS: u64 = 30;
/// Delay between multi-part message chunks (ms)
...
impl Channel for TelegramChannel {
    ...
    async fn listen(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        let mut backoff = POLL_INITIAL_BACKOFF_SECS;
        ...
        loop {
            ...
            let resp = match self
                ...
                .post(&url)
                .json(&body)
                // Outer HTTP timeout: poll timeout + a grace buffer for network latency
                .timeout(Duration::from_secs(POLL_TIMEOUT_SECS + 15))
                .send()
                .await
            {
                Ok(r) => {
                    backoff = POLL_INITIAL_BACKOFF_SECS; // reset on success
                    r
                }
                Err(e) => {
                    tracing::warn!("Telegram poll error: {e}; retrying in {backoff}s");
                    tokio::time::sleep(Duration::from_secs(backoff)).await;
                    backoff = backoff.saturating_mul(2).min(POLL_MAX_BACKOFF_SECS);
                    continue;
                }
            };

            // Telegram returns HTTP 200 even for API errors, but also returns
            // `"ok": false` in the JSON body. Handle both cases.
            if !resp.status().is_success() {
                let status = resp.status();
                let body_text = resp.text().await.unwrap_or_default();
                tracing::warn!("Telegram API HTTP {status}: {body_text}; retrying in {backoff}s");
                tokio::time::sleep(Duration::from_secs(backoff)).await;
                backoff = backoff.saturating_mul(2).min(POLL_MAX_BACKOFF_SECS);
                continue;
            }
            ...
            let data: serde_json::Value = match resp.json().await {
                Ok(d) => d,
                Err(e) => {
                    tracing::warn!("Telegram parse error: {e}; retrying in {backoff}s");
                    tokio::time::sleep(Duration::from_secs(backoff)).await;
                    backoff = backoff.saturating_mul(2).min(POLL_MAX_BACKOFF_SECS);
                    continue;
                }
            ...
            // Check Telegram-level ok flag
            if let Some(false) = data.get("ok").and_then(|v| v.as_bool()) {
                let desc = data
                    .get("description")
                    .and_then(|v| v.as_str())
                    .unwrap_or("unknown error");
                tracing::warn!("Telegram API error: {desc}; retrying in {backoff}s");
                tokio::time::sleep(Duration::from_secs(backoff)).await;
                backoff = backoff.saturating_mul(2).min(POLL_MAX_BACKOFF_SECS);
                continue;
            }
            ...
            if let Some(results) = data.get("result").and_then(serde_json::Value::as_array) {
                for update in results {
                    // Advance offset past this update — do this FIRST so we
                    // never re-process an update even if processing fails.
...
Allowlist Telegram @username or numeric user ID, then run `housaky onboard --channels-only`.",
                    ...
                    // Send "typing" indicator immediately before dispatching to agent
                    ...
                    if tx.send(msg).await.is_err() {
                        ...
                        tracing::info!(
                            "Telegram: message receiver dropped, shutting down listener"
                        );
                        return Ok(());
                    }
                }
    ...
    /// Repeats every 4.5s so the indicator stays active while the agent processes.
    ...
    /// No-op for Telegram — typing indicators expire automatically after 5s.
