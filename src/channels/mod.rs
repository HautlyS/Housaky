...
/// Maximum number of non-system turns kept in per-sender history.
...
/// Per-sender conversation history keyed by `"channel:sender"`.
/// Wrapped in `Mutex` so concurrent message workers can safely read/write.
type ConvHistoryMap = Arc<Mutex<HashMap<String, Vec<ChatMessage>>>>;
...
/// Trim a conversation history to `MAX_CHANNEL_HISTORY_TURNS` non-system turns.
...
async fn process_channel_message(ctx: Arc<ChannelRuntimeContext>, msg: traits::ChannelMessage) {
    ...
    // ── Per-sender multi-turn history ────────────────────────────────────
    // Load existing history for this sender (or create a fresh one with the
    // system prompt). We clone it out of the lock before the async LLM call
    // so we don't hold a Mutex across an await point.
    let history_key = conv_history_key(&msg);
    let mut history: Vec<ChatMessage> = {
        let mut map = ctx.conv_history.lock();
        let entry = map
    ...
    match llm_result {
        Ok(Ok(response)) => {
            ...
            // Persist updated history (with assistant reply already appended by run_tool_call_loop)
            {
                let mut map = ctx.conv_history.lock();
                let stored = map
                    .entry(history_key)
...
pub fn build_system_prompt(
    ...
    if prompt.is_empty() {
        "You are Housaky, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct.".to_string()
    } else {
        prompt
    }
}
...
pub fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: housaky channel start");
            println!("To check health:    housaky channel doctor");
            println!("To configure:      housaky onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `housaky onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.housaky/config.toml directly");
        }
    }
}
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `housaky onboard` first.");
        return Ok(());
    }

    println!("🩺 Housaky Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `housaky gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let model = config
        .default_model
        .clone()
        .unwrap_or_else(|| "arcee-ai/trinity-large-preview:free".into());
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `housaky onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 Housaky Channel Server");
    println!("  🤖 Model:    {model}");
    println!(
        "  🧠 Memory:   {} (auto-save: {})",
...
{"name":"mock_price","arguments":{"symbol":"BTC"}}
    ...
    fn prompt_injects_workspace_files() {
        ...
        assert!(prompt.contains("Name: Housaky"), "missing IDENTITY content");
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from Housaky 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
