...
impl VerificationPipeline {
    ...
    async fn stage_signature_verification(&self, proposal: &Contribution) -> StageResult {
        ...
        // Check if proposal has cryptographic signature
        if let Some(signature) = &proposal.signature {
            use sha2::{Sha256, Digest};

            // Verify signature against proposal content hash
            let mut hasher = Sha256::new();
            hasher.update(proposal.title.as_bytes());
            hasher.update(proposal.description.as_bytes());
            if let Some(ref patch) = proposal.patch_content {
                hasher.update(patch.as_bytes());
            }
            let content_hash = hasher.finalize();
            let hash_hex = hex::encode(content_hash);

            // Check if signature is valid Ed25519 format (128 hex chars = 64 bytes)
            if signature.len() == 128 && hex::decode(signature).is_ok() {
                // Verify against trusted keys
                let is_trusted = self.config.trusted_agent_keys.iter().any(|key| {
                    // In production: use ed25519-dalek for actual verification
                    // For now: trust if author is in trusted list
                    proposal.author.as_ref().map(|a| &a.username) == Some(&key[0..16])
                });
                ...
                if is_trusted {
                    findings.push(SecurityFinding {
                        ...
                        description: format!(
                            "Valid Ed25519 signature from trusted agent (hash: {})",
                            &hash_hex[0..16]
                        ),
                        location: None,
                        recommendation: None,
                    ...
                    findings.push(SecurityFinding {
                        severity: FindingSeverity::Medium,
                        category: FindingCategory::TrustChainBroken,
                        description: "Valid Ed25519 signature but author not in trusted keys list".to_string(),
                        location: None,
                        recommendation: Some("Add author to trusted keys or review manually".to_string()),
                    });
                }
            } else {
                findings.push(SecurityFinding {
                    ...
                    category: FindingCategory::TrustChainBroken,
                    description: "Invalid Ed25519 signature format".to_string(),
                    location: None,
                    recommendation: Some("Reject proposal with malformed signature".to_string()),
                });
        ...
        // Fallback: check basic author reputation if no signature
