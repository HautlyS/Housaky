...
use super::{
    ContextManager, DecompositionContext, DecompositionResult, DecompositionStrategy,
    ExecutionSummary, GSDOrchestrator, Phase, PhaseContext, PhaseStatus, TaskAwarenessReport,
    TaskStep, VerificationReport,
};
...
impl GSDExecutionEngine {
    ...
    pub async fn create_phase(
        ...
        self.orchestrator
            .create_phase(name, description, goals)
            .await
    }
    ...
    pub async fn plan_and_execute(
        ...
        let task_descriptions: Vec<String> = decomposition
            .steps
            .iter()
        ...
        self.orchestrator
            .plan_phase(phase_id, task_descriptions)
            .await?;
    ...
    pub async fn execute_with_llm(
        ...
        let context = self
            .build_planning_context(phase_id, task_description)
            .await?;

        let prompt = format!(
            r#"You are an expert task decomposition system. Analyze this task and decompose it into atomic, executable steps for an AI coding agent.
...
Respond ONLY with the XML decomposition, no other text."#,
    ...
    fn parse_llm_decomposition(
        ...
        let step_pattern = regex::Regex::new(
            r"<step>\s*<order>(\d+)</order>\s*<description>([^<]+)</description>\s*<action>([^<]+)</action>\s*(?:<files>([^<]*)</files>)?\s*<verification>([^<]+)</verification>\s*<done_criteria>([^<]+)</done_criteria>\s*(?:<dependencies>([^<]*)</dependencies>)?\s*(?:<estimated_duration_mins>(\d+)</estimated_duration_mins>)?\s*</step>",
        )?;

        for cap in step_pattern.captures_iter(response) {
            let files: Vec<String> = cap
                .get(4)
                .map(|m| {
                    m.as_str()
                        .split(',')
