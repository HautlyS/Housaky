...
const SERVICE_LABEL: &str = "com.housaky.daemon";
...
fn start(config: &Config) -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        run_checked(Command::new("systemctl").args(["--user", "start", "housaky.service"]))?;
        println!("✅ Service started");
        Ok(())
    } else {
        let _ = config;
...
fn stop(config: &Config) -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        let _ = run_checked(Command::new("systemctl").args(["--user", "stop", "housaky.service"]));
        println!("✅ Service stopped");
        Ok(())
    } else {
        let _ = config;
...
fn status(config: &Config) -> Result<()> {
    ...
    if cfg!(target_os = "linux") {
        let out =
            run_capture(Command::new("systemctl").args(["--user", "is-active", "housaky.service"]))
...
"#,
    ...
    println!("   Start with: housaky service start");
    Ok(())
}
...
fn install_linux(config: &Config) -> Result<()> {
    ...
    let unit = format!(
        "[Unit]\n\
Description=Housaky autonomous agent daemon\n\
Documentation=https://github.com/theonlyhennygod/housaky\n\
...
Restart=always\n\
RestartSec=5\n\
# Exponential back-off cap: don't restart faster than once every 5s over 1 min\n\
StartLimitInterval=60\n\
StartLimitBurst=5\n\
# Write logs to files for easy inspection\n\
StandardOutput=append:{stdout}\n\
StandardError=append:{stderr}\n\
# Ensure the process gets SIGTERM before SIGKILL\n\
TimeoutStopSec=15\n\
KillMode=mixed\n\
\n\
[Install]\n\
WantedBy=default.target\n",
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "housaky.service"]));
    println!("✅ Installed systemd user service: {}", file.display());
    println!("   Logs: {}", logs_dir.display());
    println!("   Start with: housaky service start");
    println!("   Or:         systemctl --user start housaky.service");
    Ok(())
}
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/housaky.service"));
    }
}
