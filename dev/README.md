# Housaky Development Environment

A fully containerized development sandbox for Housaky agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`housaky-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `housaky` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/housaky-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.housaky` (mounted to `/housaky-data/.housaky`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.housaky` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `housaky-local-ci` container.
