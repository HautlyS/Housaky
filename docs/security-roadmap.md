# Housaky Security Improvement Roadmap

## Current State: Strong Foundation

Housaky already has **excellent application-layer security**:
...
## Comparison: Housaky vs PicoClaw vs Production Grade

| Feature | PicoClaw | Housaky Now | Housaky + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`housaky audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/housaky/audit.log"
sign_events = true
max_size_mb = 100
...
housaky security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
housaky audit --user @alice --since 24h
housaky audit --risk high --violations-only
housaky audit --verify-signatures

# Sandbox test
housaky sandbox --test
# → Testing isolation...
...
**Housaky is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, Housaky becomes:
...
**Value**: Transforms Housaky from "safe for testing" to "safe for production"
