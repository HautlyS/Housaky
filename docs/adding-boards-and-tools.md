# Adding Boards and Tools — Housaky Hardware Guide

This guide explains how to add new hardware boards and custom tools to Housaky.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.housaky/config.toml)
housaky peripheral add nucleo-f401re /dev/ttyACM0
housaky peripheral add arduino-uno /dev/cu.usbmodem12345
housaky peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, Housaky can index PDF files:
...
2. **Add to config** — `housaky peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `housaky peripheral list` | List configured boards |
| `housaky peripheral add <board> <path>` | Add board (writes config) |
| `housaky peripheral flash` | Flash Arduino firmware |
| `housaky peripheral flash-nucleo` | Flash Nucleo firmware |
| `housaky hardware discover` | List USB devices |
| `housaky hardware info` | Chip info via probe-rs |
