...
[INFO] ✓ Audit logging enabled (~/.config/housaky/audit.log)
...
```bash
# Check what's active
$ housaky security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/housaky/audit.log
  → 47 events logged today

# Disable sandbox explicitly (writes to config)
$ housaky config set security.sandbox.enabled false

# Enable specific backend
$ housaky config set security.sandbox.backend firejail

# Adjust limits
$ housaky config set security.resources.max_memory_mb 2048
```
...
```bash
$ housaky onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Status visibility** — `housaky security --status`
