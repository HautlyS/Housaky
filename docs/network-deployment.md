# Network Deployment — Housaky on Raspberry Pi and Local Network

This document covers deploying Housaky on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | Housaky polls Telegram API; works from anywhere |
...
**Key:** Telegram, Discord, and Slack use **long-polling** — Housaky makes outbound requests. No port forwarding or public IP required.

---

## 2. Housaky on Raspberry Pi
...
- Telegram channel works: Housaky polls Telegram API (outbound)
...
```bash
housaky daemon --host 0.0.0.0 --port 8080
```
...
2. Start a tunnel:
   ...
   ```
   Or use `housaky tunnel` (see tunnel docs).

3. Housaky will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- Housaky calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `housaky daemon` — Telegram channel starts automatically.
...
- [ ] Run `housaky daemon --host 127.0.0.1 --port 8080` (Telegram works without 0.0.0.0)
