# Hardware Peripherals Design — Housaky

Housaky enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** Housaky acts as a hardware-aware AI agent that:
...
**Mental model:** Housaky = brain that understands hardware. Peripherals = arms and legs it controls.
...
Housaky runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  Housaky on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. Housaky fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
Housaky runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  Housaky on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. Housaky identifies connected hardware (VID/PID, architecture)
...
2. Housaky writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. Housaky auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| Housaky runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs Housaky; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

Housaky on Pi; GPIO via rppal or sysfs. No separate firmware.
...
housaky agent --mode edge

# Host-Mediated: connect to USB/J-Link target
housaky agent --peripheral nucleo-f401re:/dev/ttyACM0
housaky agent --probe jlink

# Hardware introspection
housaky hardware discover
housaky hardware introspect /dev/ttyACM0
```
...
board = "esp32"
transport = "wifi"
# Edge-Native: Housaky runs on ESP32
```
...
1. **Startup:** Housaky loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between Housaky and peripherals:
...
- **housaky-firmware** or **housaky-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; Housaky connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`housaky peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `housaky hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `housaky hardware introspect <path>`: memory map, peripheral list
...
- [x] Housaky on Raspberry Pi (native GPIO via rppal)
...
- [x] `housaky-esp32` firmware crate (`firmware/housaky-esp32`) — GPIO over UART
- [x] ESP32 in hardware registry (CH340 VID/PID)
- [ ] Housaky *on* ESP32 (WiFi + LLM, edge-native) — future
- [ ] Wasm or template-based execution for LLM-generated logic

**Usage:** Flash `firmware/housaky-esp32` to ESP32, add `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` to config.
...
- Running full Housaky *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). Housaky runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. Housaky gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: Housaky from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey Housaky, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
