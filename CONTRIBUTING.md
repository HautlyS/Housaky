# Contributing to Housaky

Thanks for your interest in contributing to Housaky! This guide will help you get started.
...
```bash
HOUSAKY_STRICT_LINT=1 git push
```
...
- If identity-like wording is required, use Housaky-centric labels only (for example: `Agent`, `HousakyOperator`, `housaky_user`).
...
- **Identity normalization**: when identity traits are unavoidable, use Housaky/project-native roles instead of personal or real-world identities.
...
Housaky's architecture is built on **traits** — every subsystem is swappable. This means contributing a new integration is as simple as implementing a trait and registering it in the factory function.
...
- **Identity-like labels**: if unavoidable, use Housaky-native identifiers only (`Agent`, `housaky_user`, `housaky_node`).
...
- **Good identity-like label**: `Agent`, `housaky_user`, `housaky_node`
...
- [ ] Any required identity-like wording uses Housaky/project-native labels only
