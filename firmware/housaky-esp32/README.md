# Housaky ESP32 Firmware

Peripheral firmware for ESP32 — speaks the same JSON-over-serial protocol as the STM32 firmware. Flash this to your ESP32, then configure Housaky on the host to connect via serial.
...
```sh
cd firmware/housaky-esp32
cargo build --release
espflash flash target/riscv32imc-esp-espidf/release/housaky-esp32 --monitor
```
...
Phase 6 also envisions Housaky running *on* the ESP32 (WiFi + LLM). This firmware is the host-mediated serial peripheral; edge-native will be a separate crate.
