# AGENTS.md — Housaky Agent Engineering Protocol
...
Housaky is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use Housaky-native labels only (`Agent`, `housaky_user`, `housaky_node`).
...
- If identity-like context is unavoidable, use Housaky-scoped roles/labels only (for example: `Agent`, `HousakyOperator`, `housaky_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `Agent`, `HousakyOperator`, `HousakyMaintainer`, `housaky_user`
    - service/runtime labels: `housaky_bot`, `housaky_service`, `housaky_runtime`, `housaky_node`
    - environment labels: `housaky_project`, `housaky_workspace`, `housaky_channel`
